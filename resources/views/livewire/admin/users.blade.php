...
<div class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
        <div>
            <h2 class="text-3xl font-bold text-gray-800 tracking-tight">{{ __('User Management') }}</h2>
            <p class="text-gray-500 mt-2 text-sm">{{ __('Manage your team members, assign roles, and monitor activity.') }}</p>
        </div>
        <div class="flex items-center space-x-3">
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" 
                    ...
                    " class="block w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
    ...
    <div class="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 relative overflow-hidden">
        ...
        <div class="relative z-10 flex items-center gap-3 w-full md:w-auto">
            <div class="relative w-full md:w-56 group">
                ...
                <select wire:model.live="roleFilter" class="block w-full pl-10 pr-10 py-3 text-base border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 sm:text-sm rounded-2xl bg-white text-gray-700 shadow-sm group-hover:shadow-md appearance-none transition-all">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($users as $user)
            <div class="group bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col relative transform hover:-translate-y-1">
                ...
                <div class="absolute top-4 right-4 z-10">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold {{ $user->status === 'Active' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-gray-50 text-gray-600 border border-gray-100' }} shadow-sm">
                        <span class="w-2 h-2 rounded-full {{ $user->status === 'Active' ? 'bg-green-500' : 'bg-gray-400' }} mr-2 animate-pulse"></span>
                        {{ $user->status }}
                    </span>
                </div>

                <div class="p-8 flex flex-col items-center text-center border-b border-gray-50 relative">
                    <div class="relative mb-6 group-hover:scale-105 transition-transform duration-300">
                        ...
                        <img class="h-24 w-24 rounded-2xl object-cover border-4 border-white shadow-md relative z-10"
                        ...
                        @endphp
                        <div class="absolute -bottom-3 -right-3 bg-white p-1 rounded-lg shadow-sm z-20">
                            <div class="bg-{{ $roleColor }}-100 text-{{ $roleColor }}-700 text-xs font-bold px-2 py-1 rounded-md border border-{{ $roleColor }}-200">
                                {{ $user->role }}
                            </div>
                        </div>
                    </div>
                    
                    <h3 class="text-xl font-bold text-gray-900 mb-1 tracking-tight">{{ $user->name }}</h3>
                    <p class="text-sm text-gray-500 font-medium bg-gray-50 px-3 py-1 rounded-full border border-gray-100">{{ $user->email }}</p>
                    
                    <div class="flex items-center justify-center gap-3 w-full mt-6">
                        <button wire:click="edit('{{ $user->id }}')" class="flex-1 py-2.5 px-4 bg-indigo-50 text-indigo-700 rounded-xl text-sm font-semibold hover:bg-indigo-600 hover:text-white hover:shadow-lg hover:shadow-indigo-200 transition-all duration-300 group/btn">
                        ...
                        })" class="p-2.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all duration-300 hover:scale-110">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>
                </div>
                
                <div class="px-6 py-4 bg-gray-50/30 flex flex-col gap-3 text-sm text-gray-600">
                    <div class="flex items-center justify-between group/row">
                        <span class="text-gray-400 flex items-center group-hover/row:text-indigo-500 transition-colors"><i class="fas fa-phone-alt w-5 text-center mr-2"></i> {{ __('Phone') }}</span>
                        <span class="font-medium font-mono text-gray-700">{{ $user->phone ?? '-' }}</span>
                    </div>
                    <div class="flex items-center justify-between group/row">
                        <span class="text-gray-400 flex items-center group-hover/row:text-indigo-500 transition-colors"><i class="fas fa-clock w-5 text-center mr-2"></i> {{ __('Joined') }}</span>
        ...
        @empty
            <div class="col-span-1 md:col-span-2 lg:col-span-3">
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-12 text-center relative overflow-hidden">
                    <div class="absolute inset-0 bg-grid-slate-50 [mask-image:linear-gradient(0deg,white,rgba(255,255,255,0.6))]"></div>
                    <div class="relative z-10">
                        <div class="w-24 h-24 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-6 animate-bounce">
                            <i class="fas fa-users-slash text-4xl text-indigo-400"></i>
                        </div>
                        <h3 class="text-2xl font-bold text-gray-900 mb-2">{{ __('No users found') }}</h3>
                        <p class="text-gray-500 mb-8 max-w-md mx-auto">{{ __('We couldn\'t find any users matching your search. Try adjusting your filters or create a new user to get started.') }}</p>
                        <button wire:click="create" class="inline-flex items-center px-6 py-3 bg-indigo-600 border border-transparent rounded-xl font-bold text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all duration-200 hover:-translate-y-1">
    ...
    <x-modal name="user-modal" focusable>
        <div class="bg-white rounded-3xl overflow-hidden">
            <div class="px-8 py-6 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-indigo-600 to-purple-600 text-white">
                <div>
            ...
            <form
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <!-- First Name -->
                    <div class="group">
                        <x-input-label for="first_name" value="{{ __('First Name') }}" class="text-gray-700 font-bold mb-2 ml-1" />
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <i class="fas fa-user text-gray-400 group-focus-within:text-indigo-500 transition-colors"></i>
                            </div>
                            <x-text-input wire:model="first_name" id="first_name" class="block w-full pl-10 rounded-xl border-gray-200 focus:ring-indigo-500/20 focus:border-indigo-500 py-3 transition-all" type="text"
                                placeholder="John" />
                        </div>
                    ...
                    <!-- Last Name -->
                    <div class="group">
                        <x-input-label for="last_name" value="{{ __('Last Name') }}" class="text-gray-700 font-bold mb-2 ml-1" />
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <i class="fas fa-user text-gray-400 group-focus-within:text-indigo-500 transition-colors"></i>
                            </div>
                            <x-text-input wire:model="last_name" id="last_name" class="block w-full pl-10 rounded-xl border-gray-200 focus:ring-indigo-500/20 focus:border-indigo-500 py-3 transition-all" type="text"
                                placeholder="Doe" />
                        </div>
                    ...
                    <div class="col-span-1 md:col-span-2 group">
                        ...
                        <div class="relative">
                            ...
                            <x-text-input wire:model="email" id="email" class="block w-full pl-10 rounded-xl border-gray-200 focus:ring-indigo-500/20 focus:border-indigo-500 py-3 transition-all" type="email"
                    ...
                    <!-- Phone -->
                    <div class="group">
                        <x-input-label for="phone" value="{{ __('Phone Number') }}" class="text-gray-700 font-bold mb-2 ml-1" />
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <i class="fas fa-phone text-gray-400 group-focus-within:text-indigo-500 transition-colors"></i>
                            </div>
                            <x-text-input wire:model="phone" id="phone" class="block w-full pl-10 rounded-xl border-gray-200 focus:ring-indigo-500/20 focus:border-indigo-500 py-3 transition-all" type="text"
                    ...
                    <!-- Role -->
                    <div class="group">
                        <x-input-label for="role" value="{{ __('Role') }}" class="text-gray-700 font-bold mb-2 ml-1" />
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <i class="fas fa-user-tag text-gray-400 group-focus-within:text-indigo-500 transition-colors"></i>
                            </div>
                            <select wire:model="role" id="role"
                                class="block w-full pl-10 pr-10 py-3 text-base border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 sm:text-sm rounded-xl text-gray-700 transition-all appearance-none bg-white">
                                @foreach ($roles as $roleItem)
                                    <option value="{{ $roleItem->name }}">{{ $roleItem->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <!-- Status -->
                    <div class="group">
                        <x-input-label for="status" value="{{ __('Status') }}" class="text-gray-700 font-bold mb-2 ml-1" />
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    ...
                    <div class="col-span-1 md:col-span-2 group">
                        <x-input-label for="password"
                            value="{{ $editingUserId ? __('New Password (Optional)') : __('Password') }}" class="text-gray-700 font-bold mb-2 ml-1" />
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <i class="fas fa-lock text-gray-400 group-focus-within:text-indigo-500 transition-colors"></i>
                            </div>
                            <x-text-input wire:model="password" id="password" class="block w-full pl-10 rounded-xl border-gray-200 focus:ring-indigo-500/20 focus:border-indigo-500 py-3 transition-all" type="password"
                                placeholder="********" />
                        </div>
                ...
                <div class="mt-8 flex justify-end gap-4 border-t border-gray-100 pt-6">
                    <button type="button" x-on:click="$dispatch('close-modal', 'user-modal')"
                        class="px-6 py-3 bg-white border border-gray-200 text-gray-700 rounded-xl hover:bg-gray-50 hover:text-gray-900 hover:border-gray-300 transition-all font-bold text-sm">
                        {{ __('Cancel') }}
                    </button>
                    <button type="submit"
                        class="px-6 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 hover:shadow-lg hover:shadow-indigo-200 transition-all font-bold text-sm transform hover:-translate-y-0.5">
