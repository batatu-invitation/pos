...
<div class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6 dark:bg-gray-900">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
        <div>
            <h2 class="text-3xl font-bold text-gray-800 tracking-tight dark:text-gray-100">{{ __('Suppliers') }}</h2>
            <p class="text-gray-500 mt-2 text-sm dark:text-gray-400">{{ __('Manage your product suppliers and contact information.') }}</p>
        </div>
        <div class="flex items-center space-x-3">
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" 
                     ...
                     style="display: none;">
                    <button wire:click="exportExcel" @click="open = false" class="block w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition-colors dark:text-gray-300 dark:hover:bg-gray-700">
                        <i class="fas fa-file-excel mr-2 text-green-500"></i> {{ __('Export Excel') }}
                    </button>
                    <button wire:click="exportPdf" @click="open = false" class="block w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition-colors dark:text-gray-300 dark:hover:bg-gray-700">
    ...
    <!-- Search -->
    <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 dark:bg-gray-800 dark:border-gray-700">
        <div class="relative w-full md:w-96">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <i class="fas fa-search text-gray-400"></i>
            </div>
            <input wire:model.live.debounce.300ms="search" type="text" class="block w-full pl-10 pr-3 py-2.5 border border-gray-200 rounded-xl leading-5 bg-gray-50 placeholder-gray-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 dark:placeholder-gray-500" placeholder="{{ __('Search suppliers...') }}">
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($suppliers as $supplier)
            <div class="group bg-white rounded-3xl shadow-sm border border-gray-100 p-6 flex flex-col hover:shadow-lg transition-all duration-300 relative dark:bg-gray-800 dark:border-gray-700">
                
                <!-- Status & Actions -->
                <div class="flex justify-between items-start mb-6">
                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold {{ $supplier->status === 'Active' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 dark:border-green-700' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600' }} border {{ $supplier->status === 'Active' ? 'border-green-200' : 'border-gray-200' }}">
                        {{ $supplier->status }}
                    </span>
                    
                    <div class="flex space-x-2">
                        <button wire:click="edit('{{ $supplier->id }}')" class="p-2 bg-gray-50 text-indigo-600 rounded-lg hover:bg-indigo-50 transition-colors dark:bg-gray-700 dark:hover:bg-gray-600" title="{{ __('Edit') }}">
                        ...
                        <button type="button" x-on:click="$dispatch('swal:confirm', {
                                ...
                                })" class="p-2 bg-gray-50 text-red-500 rounded-lg hover:bg-red-50 transition-colors dark:bg-gray-700 dark:hover:bg-red-900/30" title="{{ __('Delete') }}">
                ...
                <div class="flex items-center mb-6">
                    ...
                    <div>
                        <h3 class="font-bold text-gray-900 text-lg line-clamp-1 dark:text-gray-100" title="{{ $supplier->name }}">{{ $supplier->name }}</h3>
                        <div class="flex items-center text-gray-500 text-sm mt-1 dark:text-gray-400">
                ...
                <div class="space-y-3 mb-4 flex-grow">
                    <div class="flex items-start">
                        <div class="w-8 flex-shrink-0 text-center text-gray-400 dark:text-gray-500"><i class="fas fa-envelope"></i></div>
                        <span class="text-sm text-gray-600 break-all dark:text-gray-300">{{ $supplier->email ?? '-' }}</span>
                    </div>
                    <div class="flex items-start">
                        <div class="w-8 flex-shrink-0 text-center text-gray-400 dark:text-gray-500"><i class="fas fa-phone"></i></div>
                        <span class="text-sm text-gray-600 dark:text-gray-300">{{ $supplier->phone ?? '-' }}</span>
                    </div>
                    <div class="flex items-start">
                        <div class="w-8 flex-shrink-0 text-center text-gray-400 dark:text-gray-500"><i class="fas fa-map-marker-alt"></i></div>
                        <span class="text-sm text-gray-600 line-clamp-2 dark:text-gray-300">{{ $supplier->address ?? '-' }}</span>
                    </div>
                </div>
        ...
        @empty
            <div class="col-span-full">
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-12 text-center dark:bg-gray-800 dark:border-gray-700">
                    <div class="w-16 h-16 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-4 text-gray-400 dark:bg-gray-700 dark:text-gray-500">
                        <i class="fas fa-truck text-2xl"></i>
                    </div>
                    <h3 class="text-lg font-bold text-gray-900 mb-2 dark:text-gray-100">{{ __('No suppliers found') }}</h3>
                    <p class="text-gray-500 mb-6 dark:text-gray-400">{{ __('Get started by creating a new supplier.') }}</p>
                    <button wire:click="create" class="inline-flex items-center px-5 py-2.5 bg-indigo-600 border border-transparent rounded-xl font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all duration-200 dark:shadow-none">
                        <i class="fas fa-plus mr-2"></i> {{ __('Add Supplier') }}
                    </button>
                </div>
            </div>
    ...
    <x-modal name="supplier-modal" focusable>
        <div class="bg-white rounded-3xl overflow-hidden dark:bg-gray-800">
             <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50 dark:bg-gray-700/50 dark:border-gray-700">
                ...
                <button x-on:click="$dispatch('close-modal', 'supplier-modal')" class="text-gray-400 hover:text-gray-600 transition-colors dark:text-gray-400 dark:hover:text-gray-300">
